      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <Select value={currentUser} onChange={(e) => setCurrentUser(e.target.value)} style={{ padding: "5px 10px", fontSize: 13 }}>
            {players.map((p) => (
              <option key={p.id} value={p.name} style={{ background: "#111118" }}>
                {p.name} — {gameEmoji(p.game)} {p.game}
              </option>
            ))}
          </Select>
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            {[50, 100, 250, 500].map((v) => (
              <div
                key={v}
                onClick={() => setAmount(String(Math.min(v, myCoins)))}
                style={{
                  flex: 1,
...
function Classement() {
  ...
  const getStats = (name) => {
    ...
    return {
      total: myBets.length,
      wins: won.length,
      wr: finished.length > 0 ? Math.round((won.length / finished.length) * 100) : 0,
    };
  };
          ...
          <Card key={p.id} highlight={i === 0} style={{ marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{
                ...
                fontSize: i < 3 ? 24 : 16,
                fontWeight: 900,
                color: i === 0 ? T.gold : i === 1 ? "#9ca3af" : i === 2 ? "#cd7c2f" : T.faint,
                width: 34,
                textAlign: "center",
              }}>
                {i < 3 ? medals[i] : `#${i + 1}`}
    ...
    <div>
      <Card>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <Input value={newMatch.p1} onChange={(e) => setNewMatch({ ...newMatch, p1: e.target.value })} placeholder="Nom du joueur 1" />
        </div>
        <div style={{ marginBottom: 8 }}>
          <Label>JOUEUR 2</Label>
          <Input value={newMatch.p2} onChange={(e) => setNewMatch({ ...newMatch, p2: e.target.value })} placeholder="Nom du joueur 2" />
        </div>
        <div style={{ marginBottom: 12 }}>
          <Label>JEU</Label>
          <Select value={newMatch.game} onChange={(e) => setNewMatch({ ...newMatch, game: e.target.value })}>
            {GAMES.map((g) => (
              <option key={g} value={g} style={{ background: "#111118" }}>
                {gameEmoji(g)} {g}
              </option>
            ))}
          </Select>
      ...
      <Card>
        ...
        <div style={{ marginBottom: 8 }}>
          <Label>PSEUDO</Label>
          <Input value={newPlayer.name} onChange={(e) => setNewPlayer({ ...newPlayer, name: e.target.value })} placeholder="Pseudo du joueur" />
        </div>
        <div style={{ marginBottom: 12 }}>
...
export default function App() {
  ...
  return (
    <AppCtx.Provider value={{ players, setPlayers, matches, setMatches, bets, setBets, currentUser, setCurrentUser, showNotif }}>
      <Router>
        <style>{`@keyframes fadeIn { from { opacity: 0; transform: translate(-50%, -8px); } to { opacity: 1; transform: translate(-50%, 0); } } select option { background: #111118 !important; } input::placeholder { color: #555; } input[type=number]::-webkit-inner-spin-button { -webkit-appearance: none; }`}</style>
